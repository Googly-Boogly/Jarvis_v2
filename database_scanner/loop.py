...
async def run_database_loop_asynchronous():
    """
    loop for checking things in the database
    :return:
    """
    logger = create_logger_error(os.path.abspath(__file__), 'run_database_loop_asynchronous')
    ...
    while True:
        try:
            ...
            if alarm_check[0]:
                ...
                subprocess.Popen(["powershell", "-File", powershell_script], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def run_database_loop_synchronous():
    ...
    That could be turn on the lights or check alarms IE "events"
    Inside the if statements will be the code to actaully do them
    :return: Runs forever
    """
