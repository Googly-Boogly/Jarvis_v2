...
def call_function(function_name: str, info: dict, global_variables: object) -> dict:
    """
    This function will call the function that is passed in as a string. It will also pass in the info dict and the
    global variables object. It will return a dict with the anwser and the result of the function call.
    :param function_name: the name of the function you want to call
    :param info: args and quarks of the function you need to call
    :param global_variables: global vars of the project
    :return: dict with the anwser and the result of the function call
    """
    # CALENDAR
    if function_name == 'create_event':
        result = create_google_calendar_event(start_time=info['start_time'], start_date=info['start_date'], end_time=info['end_time'], end_date=info['end_date'], summary=info['summary'], description=info['description'], timezone_upload='America/Chicago', color_id=info['color_id'])
        data = {'anwser': True, 'result': result}
        return data
    elif function_name == 'edit_calendar_event':
        result = edit_calendar_event(event_id=info['event_id'], summary=info['summary'], description=info['description'], start_time=info['start_time'], end_time=info['end_time'])
        data = {'anwser': True, 'result': result}
        return data
